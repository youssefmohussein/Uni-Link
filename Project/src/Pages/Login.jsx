...
export default function Login({ onNavigateToSignup, onNavigateToForgotPassword }) {
  ...
  const isFormValid = () => email && password && validateEmail(email) && !errors.email && !errors.password;

  const handleEmailChange = (e) => {
    const value = e.target.value;
    setEmail(value);
    setErrors(prev => ({ ...prev, email: value && !validateEmail(value) ? 'Email must end with @miuegypt.edu.eg' : '' }));
  };

  const handlePasswordChange = (e) => {
    const value = e.target.value;
    setPassword(value);
    setErrors(prev => ({ ...prev, password: value && value.length < 6 ? 'Password must be at least 6 characters' : '' }));
  };
        ...
        <AuthCard>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <TextInput
                ...
                rightSlot={
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="text-[#8b949e] hover:text-[#c9d1d9]">
            ...
            <div className="flex items-center justify-end">
              <button type="button" onClick={onNavigateToForgotPassword} className="text-sm text-[#58a6ff] hover:text-[#79b8ff] font-medium hover:underline">
                Forgot Password?
              </button>
            </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-[#c9d1d9]">
              ...
              <button onClick={onNavigateToSignup} className="text-[#58a6ff] hover:text-[#79b8ff] font-semibold hover:underline">Sign Up</button>
            </p>
          </div>
        </AuthCard>
      </div>
