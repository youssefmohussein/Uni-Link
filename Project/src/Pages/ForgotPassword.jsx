...
export default function ForgotPassword({ onNavigateToLogin }) {
  ...
  const handleSubmit = (e) => {
    ...
    setIsLoading(true);
    setTimeout(() => { setIsLoading(false); setShowModal(true); }, 2000);
  };
    ...
    <>
      <div className="min-h-screen bg-[#0d1117] flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="w-full max-w-md relative">
          <AuthCard>
            <button onClick={onNavigateToLogin} className="mb-6 flex items-center space-x-2 text-[#c9d1d9] hover:text-white">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <TextInput icon={Mail} type="email" value={email} onChange={handleEmailChange} placeholder="yourname@miuegypt.edu.eg" invalid={!!error} />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-[#161b22] rounded-3xl shadow-[0_4px_12px_rgba(0,0,0,0.5)] p-8 max-w-md w-full">
            <div className="text-center">
              ...
              <p className="text-[#c9d1d9] mb-6">We've sent a password reset link to <span className="font-semibold text-[#58a6ff]">{email}</span></p>
              <button onClick={handleCloseModal} className="w-full py-4 rounded-xl font-semibold text-white bg-gradient-to-r from-[#58a6ff] to-[#79b8ff] hover:from-[#388bfd] hover:to-[#58a6ff] transition-all">Back to Login</button>
            </div>
          </div>
        </div>
      )}
