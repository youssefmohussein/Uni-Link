...
const BentoCardGrid = ({ children, gridRef }) => (
    <div
        className="bento-section grid gap-4 p-6 max-w-[75rem] select-none relative"
        style={{ fontSize: 'clamp(1.1rem, 1rem + 0.6vw, 1.7rem)' }}
        ref={gridRef}
    >
        {children}
    </div>
            ...
            <BentoCardGrid gridRef={gridRef}>
                <div className="card-responsive grid gap-4">
                    {cardData.map((card, index) => {
                        ...
                        const baseClassName = `card flex flex-col justify-between relative aspect-[4/3] min-h-[240px] w-full max-w-full p-6 rounded-[24px] border border-solid font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-1 ${enableBorderGlow ? 'card--border-glow' : ''
                            }`;
                            ...
                            return (
                                ...
                                >
                                    <div className="card__header flex justify-between items-center gap-3 relative text-white">
                                        <span className="card__label text-sm opacity-80">{card.label}</span>
                                    </div>
                                    <div className="card__content flex flex-col items-center justify-center relative text-white text-center flex-1">
                                        <Icon className="card__icon" style={{ color: card.iconColor }} />
                                        <h3 className={`card__title font-semibold text-xl m-0 mb-2 ${textAutoHide ? 'text-clamp-1' : ''}`}>
                                            {card.title}
                                        </h3>
                                        <p className={`card__description text-sm leading-6 opacity-75 ${textAutoHide ? 'text-clamp-2' : ''}`}>
                                            {card.description}
                                        </p>
                                    </div>
                                </ParticleCard>
