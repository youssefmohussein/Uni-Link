...
function ProjectCard({ project_id, title, description, status, grade, skills, supervisor_name, created_at, file_path, onDelete, onEdit, userId }) {
  ...
  return (
    <article
      className="relative overflow-hidden border border-white/10 bg-white/5 rounded-2xl"
    >
      {/* Status Badge */}
      {status && (
        ...
        <div className="flex-1 px-20 py-10">
          <div className="flex flex-col justify-center space-y-5 max-w-5xl">
            <div className="space-y-3">
              ...
              <p className="text-base text-gray-300 leading-relaxed line-clamp-3 max-w-4xl">{description}</p>
            </div>
            ...
            <div className="flex items-center gap-12 text-sm flex-wrap">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-white font-semibold">{supervisor_name || '—'}</span>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-gray-400 min-w-[50px]">Grade:</span>
                <span className="text-accent font-bold text-base">{grade !== null && grade !== undefined ? grade : '—'}</span>
              </div>
            </div>
          </div>
        </div>
